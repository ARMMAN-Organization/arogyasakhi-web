...
describe('authSlice', () => {
  it('starts with no token, refresh token, or user', () => {
    ...
    expect(state).toEqual({ token: null, refreshToken: null, expiresIn: null, user: null });
  });

  it('stores credentials on setCredentials', () => {
    const state = authReducer(
      undefined,
      setCredentials({ token: 'tok', refreshToken: 'rt', expiresIn: 900, user }),
    );
  ...
  it('a later setCredentials (e.g. token refresh) keeps existing profile fields', () => {
    ...
    const refreshed = authReducer(
      withProfile,
      setCredentials({ token: 'tok2', refreshToken: 'rt2', expiresIn: 300, user }),
    );
